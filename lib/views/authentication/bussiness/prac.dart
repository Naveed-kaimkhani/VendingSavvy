...
class _ExpensemanagmentState extends State<Expensemanagment>
  ...
  double getTotalValue() {
    return pieChartSections.map((section) => section.value).reduce((a, b) => a + b);
  }
  ...
  Widget _buildCategoryItem(
    ...
    double percentageValue = double.tryParse(percentage.replaceAll(RegExp(r'[+%-]'), '')) ?? 0.0;
    // Normalize to a value between 0.0 and 1.0 (assuming max percentage is 10% for scaling)
    double fillValue = percentageValue.clamp(0.0, 10.0) / 10.0; // Scales 0% to 10% to 0.0 to 1.0
        ...
        children: [
          ...
          LinearProgressIndicator(
            ...
            valueColor: AlwaysStoppedAnimation<Color>(
              percentage.contains('-') ? Colors.red.withOpacity(0.5) : Colors.green.withOpacity(0.5),
            ),
            minHeight: 4.h, // Adjusted height for visibility
          ),
        ],
      ),
    );
